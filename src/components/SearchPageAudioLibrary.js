...
class SearchPage extends Component {
  ...
  onChangeSearch(event, data) {
    ...
    let wordsList = fuzzysort.go(new_search, audiolibrary, optionsFuzzy).map(({ obj }) => (obj));
  ...
  const panes = [
    ...
    return (
          <Grid>
            ...
            {displayList  ? 
                wordsList.map((word) => 
                <List.Item onClick={()=>this.repeatAudio(word)}>
                <AudioItem key={word} word={word} search={this.state.search} wordsList={this.state.wordsList} />
                </List.Item>
                )
              : 
