...
class SearchPage extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      endingIndexes: props.location.state === undefined ? [] : props.location.state.endingIndexes,
      yugtun: props.location.state === undefined ? [] : props.location.state.yugtun,
      english: props.location.state === undefined ? [] : props.location.state.english,
      links: props.location.state === undefined ? [] : props.location.state.links,
      ...
      searchBarStuckTop:
        props.location.state === undefined ? false : props.location.state.searchBarStuckTop,
      notFirstParse:
        props.location.state === undefined ? false : props.location.state.notFirstParse,
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.startingSearch && !this.state.startingSearch) {
      if (navigator.userAgent.match(/(iPhone|iPod|iPad)/i)) {
        let elts =
          ReactDOM.findDOMNode(this).getElementsByClassName("search_container");
        if (elts.length > 0) {
  ...
  getParse(word) {
    if (word === "") {
      ...
      axios.get(API_URL + "/parseV2/" + word).then((response) => {
  ...
  handleClick2 = (e, titleProps) => {
    ...
    this.setState({
      loaderOn: false,
      entries: mood === "[DemPro]" ? demPro : perPro,
  ...
  onKeyPress = (e) => {
    if (e.key === "Enter") {
      //} && (this.state.activeTabIndex === 0 || this.state.search !== this.state.searchWord)){
  ...
  endingToEnglish(ending, index, qindex) {
    ...
    if (ending.includes("[V]")) {
      if (
        this.state.parses[index].includes("[Ind]") ||
      ...
      if (ending.includes("[Trns]")) {
        ...
        english3 +=
          subject + " to " + endingToEnglishTerms[tags[tags.length - 1]];
      ...
      if (ending.includes("Poss")) {
        english3 +=
          endingToEnglishTerms[tags[tags.length - 2]] +
          "\xa0" +
          endingToEnglishTerms[tags[tags.length - 1]];
      } else {
        english3 += endingToEnglishTerms[tags[tags.length - 1]];
      }
    ...
    return (
      <div style={{ paddingTop: 15, paddingLeft: 20 * qindex }}>
        <div style={{ fontWeight: "bold", fontSize: 16, paddingBottom: "1px" }}>
          {this.state.yugtun[index][qindex]}
        </div>
        <div style={{ fontSize: 16 }}>
          {before && english4.length !== 0 ? (
            <span>{english4 + "\xa0"}</span>
          ) : null}
          <span>{english3}</span>
          {!before ? <span>{"\xa0" + english4}</span> : null}
        </div>
        <div
          onClick={() =>
            this.setState({
              showMoreEnding:
                this.state.moreIndex !== index || !this.state.showMoreEnding
                  ? true
                  : false,
              moreIndex: index,
            })
        ...
        {this.state.showMoreEnding && index === this.state.moreIndex ? (
          <div style={{ marginLeft: 15, fontSize: 16 }}>
            ...
            <div>{english1}</div>
            <div
              onClick={() => window.open("http://www.yugtun.com/symbols")}
              style={{
                color: "#4183c4",
                fontWeight: "100",
  ...
  //     this.setState({ yugtunAnalyzer: false, activeTabIndex:0,parses:[],segments:[],endingIndexes:[],newSearchList:[],notFirstParse:false,search:""})
  //   } else if (data.activeIndex === 1 && this.state.activeTabIndex !== 1) {
  //     // if (!this.state.yugtunAnalyzer) {
  //     this.setState({ yugtunAnalyzer: true, activeTabIndex:1, parses:[],segments:[],endingIndexes:[],newSearchList:[],notFirstParse:false,search:""});
  //     // this.inputClicked()
  //     // }
  //   }
  // }
  ...
  getLinks(index, parse) {
    ...
    if (index === 0) {
      // if base
      if (
        (parse[index].includes("[P") || parse[index].includes("[I")) &&
        parse.length === 1
      ) {
        // if particle or ignorative
        return parse[index].split("[")[0].replace(/=/g, "-");
      } else if (parse[index].includes("[PerPro]")) {
        return parse[index].split("[")[0];
      } else if (
        parse[index].includes("[DemPro]") ||
        parse[index].includes("[DemAdv]")
      ) {
  ...
  render() {
    ...
    return (
      <div>
                  ...
                  onClick={() => {
                    if (index !== this.state.activeSentenceIndex) {
                      this.setState({
                        ...
                        searchWord: this.state.newSearchList[index]
                          .replace(/[^a-zA-Z\-̄͡͞ńḿ']/g, "")
                          .toLowerCase(),
                        parses: [],
                        segments: [],
                      ...
                      this.getParse(
                        this.state.newSearchList[index]
                          .replace(/[^a-zA-Z\-̄͡͞ńḿ']/g, "")
                          .toLowerCase()
                      );
                    }
        ...
        !this.state.getCall &&
        this.state.notFirstParse ? (
          <div style={{ paddingTop: 20 }}>
            <Divider />
            <div style={{ fontStyle: "italic", marginTop: 10 }}>
              No results. Pisciigatuq. Type a complete Yugtun word.
            </div>
          </div>
        ) : null}
        ...
        {pressEnter ? (
          <p>
            <i>
              Press <b>Enter</b> to parse. Must be complete Yugtun word(s).
            </i>
          </p>
        ) : (
          ""
            ...
            {this.state.parses.map((i, index) => (
              <div>
                ...
                {i.split("-").map((q, qindex) =>
                  qindex === this.state.endingIndexes[index] ? (
                    this.endingToEnglish(q, index, qindex)
                  ) : (
                    <div
                      style={{ paddingTop: 15, paddingLeft: qindex * 20, fontSize: "16px", }}
                    >
                      {this.state.links[index][qindex] !== "" ? (
                        <div>
                          ...
                          >
                            <Link
                              to={{
                                pathname: this.state.links[index][qindex],
                                state: {
                                  word: this.state.links[index][qindex],
                                  search: this.state.search,
                                  newSearchList: this.state.newSearchList,
                                  parses: this.state.parses,
                                  segments: this.state.segments,
                                  endingIndexes: this.state.endingIndexes,
                                  yugtun: this.state.yugtun,
                                  english: this.state.english,
                                  links: this.state.links,
                                  searchBarStuckTop: this.state.searchBarStuckTop,
                            ...
                            >
                              <span>{this.state.yugtun[index][qindex]}</span>
                            </Link>
                          </div>
                          {
                            this.state.english[index][qindex]
                          }
                        </div>
                      ) : (
                        <div
                          style={{ fontWeight: "bold", fontFamily: "Lato,Arial,Helvetica,sans-serif", textDecoration: "underline", }}
                        >
                          {this.state.yugtun[index][qindex]}
                        </div>
                      )}
                ...
                >
                  {!this.state.parses[index].includes("-") &&
                  (this.state.parses[index].includes("[I") ||
                    this.state.parses[index].includes("[P")) ? null : (
                    <Button
                      basic
                      color="blue"
                      ...
                      onClick={() => {
                        this.setState({
                          currentTableOpen:
                            this.state.currentTableOpen === index ? -1 : index,
                          activeIndex: -1,
                        });
                      }}
                    >
                        ...
                        <div>
                          ...
                          <Accordion.Content active={activeIndex === pindex}>
                            <div style={{ paddingBottom: 15 }}>
                              {endingDescriptions[pindex]}
                            </div>
                            <div
                              style={{ paddingBottom: 10, fontStyle: "italic" }}
                            >
                              {"Examples: " + exampleSentences[pindex]}
                            </div>
                            {this.state.loaderOn ? (
                              <Loader active inline />
                            ) : (
                              <div>
                                ...
                                />
                                <div
                                  style={{ fontStyle: "italic", paddingTop: 15, paddingBottom: 5, }}
                                >
                                  {
                                    "Note: Many verb bases can be intransitive-only, transitive-only, or only allow certain endings. There are occasional mistakes."
                                  }
                                </div>
                              </div>
                            )}
                    ...
                    <Accordion style={{ fontSize: 16 }} fluid styled>
                      {accordionTitlesNouns.map((p, pindex) => (
                        <div>
                          ...
                          >
                            <div style={{ paddingBottom: 15 }}>
                              {endingDescriptions[pindex + 12]}
                            </div>
                            <div
                              style={{ paddingBottom: 15, fontStyle: "italic" }}
                            >
                              {"Examples: " + exampleSentences[pindex + 12]}
                            </div>
                            {this.state.loaderOn ? (
                                      ...
                                      {persPronounSubjects.map((i, index) => (
                                        <Table.Row>
                                          <Table.HeaderCell
                                            style={{ color: "#002477" }}
                                          >
                                            {persPronounsEnglish[index]}
                                          </Table.HeaderCell>
                                          <Table.Cell
                                            style={{ paddingLeft: 10 }}
                                          >
                                            {this.state.entries[
                                              persPronounLabels[pindex] + i
                                            ]
                                              .join(",\n")
                                              .replace(/>/g, "")}
                                          </Table.Cell>
                        ...
                        <div>
                          ...
                          >
                            {this.state.loaderOn ? (
                              ...
                              <div>
                                <div
                                  style={{ fontStyle: "italic", marginTop: 5, marginBottom: 10, }}
                                >
                                  Extended: moving, long, or of large extent
                                </div>
                                <div
                                  style={{ fontStyle: "italic", marginTop: 10, marginBottom: 10, }}
                                >
                                  Restricted: stationary, localized, visible
                                </div>
                                <div
                                  style={{
                                    fontStyle: "italic",
                                ...
                                >
                                  Obscured: stationary, indistinct or out of
                                  sight
                                </div>
                                <Segment
                                  style={{ margin: 0, overflow: "auto" }}
                                        ...
                                        index === 24 ? (
                                          ...
                                          <Table.Row>
                                            <Table.HeaderCell
                                              style={{ color: "#002477" }}
                                            >
                                              {demPronounEnglish[index]}
                                            </Table.HeaderCell>
                                            {"extended" in
                                            this.state.entries[p] ? (
                                              <Table.Cell
                                                style={{ paddingLeft: 10 }}
                                              >
                                                {this.state.entries[p][
              ...
              <div>
                ...
                >
                  <Image
                    ...
                    src={
                      "https://yupikmodulesweb.s3.amazonaws.com/static/exercise1/ellanguaq1.svg"
                    }
                  />
                  <Image
                    style={{ height: "30px", opacity: 0.7 }}
                ...
                >
                  <div>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      {" "}
                      Type any complete Yugtun word or sentence and press{" "}
                      <b>Enter</b> to see the meaning of each part of the word.{" "}
                    </div>
                    <div> Examples: </div>
                    <div>
                      <span
                        onClick={() => {
                          ...
                          this.inputClicked("elitnaurvigmi");
                        }}
                        style={{ textDecoration: "underline", color: "#4A80B5", cursor: "pointer", }} 
                      >
                        elitnaurvigmi
                      </span>
                      <span>{", "}</span>
                      <span
                        onClick={() => {
                          ...
                          this.inputClicked("piipiqa popsicle-aamek ner'uq");
                        }}
                        style={{ textDecoration: "underline", color: "#4A80B5", cursor: "pointer", }} 
                      >
                        piipiqa popsicle-aamek ner'uq
                      </span>
                      <span>{", "}</span>
                      <span
                        onClick={() => {
                          ...
                          this.inputClicked("ellami-lli assirpaa");
                        }}
                        style={{ textDecoration: "underline", color: "#4A80B5", cursor: "pointer", }}
                      >
                        ellami-lli assirpaa
