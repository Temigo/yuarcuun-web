  ...
  handleClick1 = (index,allowAdd) => {
	...
	subMenuItem=(type, ind1, ind2)=> {
	    ...
	    return <Accordion style={{color:'000000de',borderTop:'1px solid #2224261a',padding:'4px 9px',paddingBottom:'9px'}}>
		    		...
		    		<Button.Group vertical basic fluid>
	        <Button onClick={()=>{this.editMenuRef[this.state.currentlyOpen].close(); this.setState({currentlyOpen:'',cvvMood:'Prec',cvvType:'',isOpen: false},()=>{this.backEndCall([["Update",["cv","vMood"],this.state.cvvMood]])})}}>before...</Button>
	        <Button onClick={()=>{this.editMenuRef[this.state.currentlyOpen].close(); this.setState({currentlyOpen:'',cvvMood:'Cnsq',cvvType:'',isOpen: false},()=>{this.backEndCall([["Update",["cv","vMood"],this.state.cvvMood]])})}}>because...</Button>
	        <Button onClick={()=>{this.editMenuRef[this.state.currentlyOpen].close(); this.setState({currentlyOpen:'',cvvMood:'Cont',cvvType:'',isOpen: false},()=>{this.backEndCall([["Update",["cv","vMood"],this.state.cvvMood]])})}}>whenever...</Button>
	        <Button onClick={()=>{this.editMenuRef[this.state.currentlyOpen].close(); this.setState({currentlyOpen:'',cvvMood:'Conc',cvvType:'eventhough',isOpen: false},()=>{this.backEndCall([["Update",["cv","vMood"],this.state.cvvMood],["Update",["cv","vType"],this.state.cvvType]])})}}>even though...</Button>
	    ...
	    return <Accordion style={{color:'000000de',borderTop:'1px solid #2224261a',padding:'4px 9px',paddingBottom:'9px'}}>
	            ...
	            onClick={()=>{this.handleClick(type,true)}}
			        ...
			        <Button onClick={()=>{this.editMenuRef[this.state.currentlyOpen].close(); this.setState({currentlyOpen:'',optCase:'Sbrd',mvvType:'neg',isOpen: false},()=>{this.backEndCall([["Update",["mv","vMood"],'Sbrd'],["Insert",["mv"],[this.state.mvvBase[0].concat([['+peke-|+vke-,+pege-|+vke-', 0, 0, 0]]),this.state.mvvBase[1],'Sbrd']]])})}}>polite do not...</Button>
		    		</Button.Group>
		    		</Accordion.Content>
	    		</Accordion>		
		...
		if (key['type'] == 'n') {
			...
			if (this.state.nextTenses.length > 0) {
				if ((this.state.addIs && key['englishtenses'][2]=='is') || this.state.nextTenses[this.state.nextTenses.length-1] === 'gen') {
					...
					if (this.state.nextTenses[this.state.nextTenses.length-1] === 'p') {
						if (['Intrg4','Intrg5','Intrg6','Intrg7','Intrg8','Intrg9','IntrgA'].includes(this.state.mvvType)) {
		...
		if (lookup in newpostbases) {
			...
			if (newpostbases[lookup]['type'] === 'VV') {
				...
				if (key['id'] == 5) {
					...
					// if (this.state.nextTenses.length === 0 && mood === {
						// let s1 = ''
						// if (vsPlanned[0] == 1 && vsPlanned[1] == 1)	{
						// 	sentence = 'am '
						...
						// }
					// }
					// if (specialStartingCase[1].length > 0) {
						// if (this.state.nextTenses.length == 0) {
							// s1 = specialStartingCase[1]+' '
						// } 
						// else if (this.state.nextTenses[this.state.nextTenses.length-1] === 'p') {
							// s1 = specialStartingCase[1].replace('is','did').replace('are','do').replace('am','did')+' '
						// }
					// }
					sentence = s1+'not'
				} else {
		...
		if (itemUpdating[0]==='Insert') {
			if (mood==='Ind') {
				...
				if (submood in starters) {
					...
					if (this.state.nextTenses.length > 0) {
						if (this.state.nextTenses[this.state.nextTenses.length-1] === 'p') {
							subject = subject.replace('*are*','did')
						}
					}
					subject = subject.replace('*are*','are')
				}
			} else {
				if (mood in starters) {
																		    		...
																		    		nextTenses:(
																		    			newpostbases[k['fsts']]['match_case'] && !('gen_preverb_after' in newpostbases[k['fsts']]) ? 
																			    			(this.state.nextTenses.length == 0 ? 
																			    				this.state.nextTenses.concat([this.returnBaseCaseforMoods(itemUpdating[1][0],mood,vsPlanned)[0]])
																			    				:
																			    				this.state.nextTenses.concat([this.state.nextTenses[this.state.nextTenses.length-1]])
																			    			) 
																		    			: 				
																								(newpostbases[k['fsts']]['match_case'] ? 
																				    			(this.state.nextTenses.length == 0 ? 
																				    				this.state.nextTenses.concat([this.returnBaseCaseforMoods(itemUpdating[1][0],mood,vsPlanned)[0]])
																				    				:
																				    				this.state.nextTenses.concat([this.state.nextTenses[this.state.nextTenses.length-1]])
														      				...
														      				candidateBase:this.state.candidateBase.concat([[newpostbases[popularPostbases[p]['expression']]['keylookup'],popularPostbases[p]['expression']]]),
																	    		...
																	    		(newpostbases[popularPostbases[p]['expression']]['match_case'] && !('gen_preverb_after' in newpostbases[popularPostbases[p]['expression']]) 
																	    			...
																	    			(this.state.nextTenses.length == 0 
																	    				...
																	    				this.state.nextTenses.concat([this.state.nextTenses[this.state.nextTenses.length-1]])) 
																	    			: 
																	    			(newpostbases[popularPostbases[p]['expression']]['match_case'] ?
																	    				(this.state.nextTenses.length == 0 ?
																		    				this.state.nextTenses.concat([this.returnBaseCaseforMoods(itemUpdating[1][0],mood,vsPlanned)[0]])
																	    					:
																	    					this.state.nextTenses.concat([this.state.nextTenses[this.state.nextTenses.length-1]])
